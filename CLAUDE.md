...
"High Orbit": a bright space aesthetic, dark-first with a full light theme — dusk orbit (indigo→violet gradient, faint static starfield, frosted-glass cards, glowing status stripes) and dawn orbit (lavender→peach gradient, stars off via transparent tokens, frosted-white cards, deepened status colors). Token-level three-state theme pattern: bare `:root` = dark, `@media (prefers-color-scheme: light)` guarded `:root:not([data-theme="dark"])`, plus `:root[data-theme="light"]`. Fonts: Chakra Petch (signage/wordmark), Sora (body), JetBrains Mono (telemetry). Aurora accent pair `--accent-a` (teal) → `--accent-b` (violet) drives the wordmark underscore, header beam, and disclosure carets; glows are tokens (`--stripe-blur`, `--hot-glow`, `--dot-glow`) so light mode softens them. Status colors: idea periwinkle, active mint GO-green, parked HOLD-amber, shipped sky-blue, dropped grey-violet. Mission language unchanged: callsigns `[PROPOSED] [GO] [COMPLETE] [HOLD] [SCRUBBED]`, `HOLD T+<n>d` pills, "Mission complete" stamps, "clearance to resume" phrasing. Checklists/notes are native `<details>`, collapsed by default. Days-parked is computed live in the page from `parked_since` — never bake day counts into the blob. (The original CRT-console design was replaced 2026-09-10; per-board pages pick the new design up automatically on their next data change via template re-rendering.)
